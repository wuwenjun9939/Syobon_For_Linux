//-----------------------------------------------------------------------------
// 
// 		ＤＸライブラリ		Linux用GraphFilter系プログラム
// 
//  	Ver 3.24f
// 
//-----------------------------------------------------------------------------
...
GRAPHFILTER_SYSTEMIFNO_LINUX GraphFilterSystemInfoLinux ;

// 関数宣言 -------------------------------------------------------------------

static int		LINUX_GraphFilter_GetWorkTexture( int IsFloatType, DWORD TexSizeW, DWORD TexSizeH, DWORD HandleIndex ) ;	// フィルター作業用のテクスチャハンドルを取得する
static int		LINUX_MemLoadShaderCode( const char *ShaderName, int ShaderType ) ;										// 指定名の事前用意シェーダーを読み込む
static GLuint	LINUX_GetFragmentShader( int PixelShaderHandle ) ;														// ピクセルシェーダーハンドルの中のフラグメントシェーダーを取得する
static int		LINUX_FilterStretchBlt( GRAPHICS_LINUX_SHADER *UseShader, GRAPHFILTER_INFO *Info, int IsLinearFilter = TRUE, int ScaleDivNum = 1, int SubImageHandle = -1, VERTEX_TEX8_2D *Texcoord8Vertex = NULL ) ;

// プログラム -----------------------------------------------------------------

// フィルター作業用のテクスチャハンドルを取得する
static int	LINUX_GraphFilter_GetWorkTexture( int IsFloatType, DWORD TexSizeW, DWORD TexSizeH, DWORD HandleIndex )
{
	IMAGEDATA *BaseImage ;
	DWORD       NPowW ;
	DWORD       NPowH ;
	...
	if( GRAPHCHK( GraphFilterSystemInfoLinux.WorkDrawValidGrHandle[ IsFloatType ][ NPowW ][ NPowH ][ HandleIndex ], BaseImage ) )
	{
		...
		GraphFilterSystemInfoLinux.WorkDrawValidGrHandle[ IsFloatType ][ NPowW ][ NPowH ][ HandleIndex ] = Graphics_Image_MakeGraph_UseGParam( &GParam, WorkTexSizeW, WorkTexSizeH, FALSE, FALSE, 0, FALSE ) ;
		if( GraphFilterSystemInfoLinux.WorkDrawValidGrHandle[ IsFloatType ][ NPowW ][ NPowH ][ HandleIndex ] < 0 )
		{
		...
		NS_SetDeleteHandleFlag( GraphFilterSystemInfoLinux.WorkDrawValidGrHandle[ IsFloatType ][ NPowW ][ NPowH ][ HandleIndex ], &GraphFilterSystemInfoLinux.WorkDrawValidGrHandle[ IsFloatType ][ NPowW ][ NPowH ][ HandleIndex ] ) ;
	}

	return GraphFilterSystemInfoLinux.WorkDrawValidGrHandle[ IsFloatType ][ NPowW ][ NPowH ][ HandleIndex ] ;
}

// 指定名の事前用意シェーダーを読み込む
static int LINUX_MemLoadShaderCode( const char *ShaderName, int ShaderType )
{
	int Addr, Size ;
	BYTE *FileImage ;
	int Ret ;

	if( GLINUX.ShaderCode.Base.BaseShaderInitializeFlag == FALSE )
	{
		return -1 ;
	}

	if( DXA_GetFileInfo( &GLINUX.ShaderCode.Base.FilterShaderBinDxa, DX_CHARCODEFORMAT_ASCII, ShaderName, &Addr, &Size ) < 0 )
	{
		return -1 ;
	}

	FileImage = ( BYTE * )DXA_GetFileImage( &GLINUX.ShaderCode.Base.FilterShaderBinDxa ) + Addr ;
...
static GLuint LINUX_GetFragmentShader( int PixelShaderHandle )
{
...
static int LINUX_FilterStretchBlt( GRAPHICS_LINUX_SHADER *UseShader, GRAPHFILTER_INFO *Info, int IsLinearFilter, int ScaleDivNum, int SubImageHandle, VERTEX_TEX8_2D *Texcoord8Vertex )
{
	...
	{
		DestFrameBuffer       = GLINUX.Device.Screen.SubBackBufferFrameBuffer ;
		DestFrameBufferWidth  = GLINUX.Device.Screen.SubBackBufferTextureSizeX ;
		DestFrameBufferHeight = GLINUX.Device.Screen.SubBackBufferTextureSizeY ;
	}
...
{
	// 全てのシェーダーを解放する
	GraphFilter_Linux_ReleaseShaderAll() ;
...
extern int	GraphFilter_Mono_PF(       GRAPHFILTER_INFO *Info, float Cb, float Cr )
{
	...
	DX_LINUX_SHADER_FLOAT4 ParamF4[ 2 ] ;
	GRAPHICS_LINUX_SHADER *UseAndrShader ;

	// 使用するシェーダーのセットアップ
	if( GraphFilterShaderHandle.MonoPS < 0 )
	{
		GraphFilterShaderHandle.MonoPS = LINUX_MemLoadShaderCode( FlagFileName[ 0 ], DX_SHADERTYPE_PIXEL ) ;
		if( GraphFilterShaderHandle.MonoPS < 0 )
		{
	...
	if( GraphFilterSystemInfoLinux.Mono.Shader == 0 )
	{
		Graphics_Linux_Shader_Create( &GraphFilterSystemInfoLinux.Mono, GraphicsHardDataLinux.Device.Shader.Base.StretchRect_VS, LINUX_GetFragmentShader( GraphFilterShaderHandle.MonoPS ) ) ;
	}
	UseAndrShader = &GraphFilterSystemInfoLinux.Mono ;
	...
	// Uniform の値をセット
	UNIFORM_SET_INT1(   Graphics_Linux_Shader_GetUniformIndex( UseAndrShader, "uSrcTex"     ), 0            ) ;
	UNIFORM_SET_FLOAT4( Graphics_Linux_Shader_GetUniformIndex( UseAndrShader, "uRGBToY"     ), ParamF4[ 0 ] ) ;
	UNIFORM_SET_FLOAT4( Graphics_Linux_Shader_GetUniformIndex( UseAndrShader, "uYCbCrToRGB" ), ParamF4[ 1 ] ) ;

	LINUX_FilterStretchBlt( UseAndrShader, Info, FALSE ) ;
...
/*	if( Param < 0.0000001f )
	...
	{
		DX_LINUX_SHADER_FLOAT4 ParamF4Buf[ 12 ] ;
		DX_LINUX_SHADER_FLOAT4 *ParamF4 = ParamF4Buf ;
		GRAPHICS_LINUX_SHADER *UseAndrShader ;

		// 使用するシェーダーのセットアップ
		if( GraphFilterShaderHandle.Gauss_PS[ UseShader ] < 0 )
		{
			GraphFilterShaderHandle.Gauss_PS[ UseShader ] = LINUX_MemLoadShaderCode( FlagFileName[ UseShader ], DX_SHADERTYPE_PIXEL ) ;
			if( GraphFilterShaderHandle.Gauss_PS[ UseShader ] < 0 )
			{
				char PathUTF16LE[ 128 ] ;
		...
		if( GraphFilterSystemInfoLinux.Gauss[ UseShader ].Shader == 0 )
		{
			Graphics_Linux_Shader_Create( &GraphFilterSystemInfoLinux.Gauss[ UseShader ], GraphicsHardDataLinux.Device.Shader.Base.StretchRect_VS, LINUX_GetFragmentShader( GraphFilterShaderHandle.Gauss_PS[ UseShader ] ) ) ;
		}
		UseAndrShader = &GraphFilterSystemInfoLinux.Gauss[ UseShader ] ;
		...
		// Uniform の値をセット
		UNIFORM_SET_INT1(   Graphics_Linux_Shader_GetUniformIndex( UseAndrShader, "uSrcTex"      ), 0            ) ;
		glUniform4fv( Graphics_Linux_Shader_GetUniformIndex( UseAndrShader, "uPixelWeight" ), 4, ParamF4Buf[ 0 ] ) ;
		glUniform4fv( Graphics_Linux_Shader_GetUniformIndex( UseAndrShader, "uPixelOffset" ), 8, ParamF4Buf[ 4 ] ) ;

		LINUX_FilterStretchBlt( UseAndrShader, Info ) ;
...
extern int	GraphFilter_Down_Scale_PF(  GRAPHFILTER_INFO *Info, int DivNum )
{
	...
	int                    UseShader = 0 ;
	int                    UseConstNum = 0 ;
	DX_LINUX_SHADER_FLOAT4  ParamF4[ 8 ] ;
	int                    TextureW ;
	int                    TextureH ;
	float                  TextureWF ;
	float                  TextureHF ;
	...
	{
		GraphFilterShaderHandle.DownScalePS[ UseShader ] = LINUX_MemLoadShaderCode( FlagFileName[ UseShader ], DX_SHADERTYPE_PIXEL ) ;
		if( GraphFilterShaderHandle.DownScalePS[ UseShader ] < 0 )
		{
			char PathUTF16LE[ 128 ] ;
	...
	if( GraphFilterSystemInfoLinux.DownScale[ UseShader ].Shader == 0 )
	{
		Graphics_Linux_Shader_Create( &GraphFilterSystemInfoLinux.DownScale[ UseShader ], GraphicsHardDataLinux.Device.Shader.Base.StretchRect_VS, LINUX_GetFragmentShader( GraphFilterShaderHandle.DownScalePS[ UseShader ] ) ) ;
	}
	UseAndrShader = &GraphFilterSystemInfoLinux.DownScale[ UseShader ] ;
	...
	// Uniform の値をセット
	UNIFORM_SET_INT1(   Graphics_Linux_Shader_GetUniformIndex( UseAndrShader, "uSrcTex"      ), 0               ) ;
	glUniform4fv(       Graphics_Linux_Shader_GetUniformIndex( UseAndrShader, "uPixelOffset" ), 8, ParamF4[ 0 ] ) ;

	LINUX_FilterStretchBlt( UseAndrShader, Info, TRUE, DivNum ) ;
...
extern int	GraphFilter_Bright_Clip_PF( GRAPHFILTER_INFO *Info, int CmpType, float CmpParam, int ClipFillFlag, COLOR_F *ClipFillColor, int IsPMA )
{
	...
	int UseShader = 0 ;
	int UseFill ;
	DX_LINUX_SHADER_FLOAT4  ParamF4[ 3 ] ;
	GRAPHICS_LINUX_SHADER *UseAndrShader ;
	...
	{
		GraphFilterShaderHandle.BrightClipPS[ UseShader ][ UseFill ][ IsPMA ] = LINUX_MemLoadShaderCode( FlagFileName[ UseShader ][ UseFill ][ IsPMA ], DX_SHADERTYPE_PIXEL ) ;
		if( GraphFilterShaderHandle.BrightClipPS[ UseShader ][ UseFill ][ IsPMA ] < 0 )
		{
			char PathUTF16LE[ 128 ] ;
	...
	if( GraphFilterSystemInfoLinux.BrightClip[ UseShader ][ UseFill ][ IsPMA ].Shader == 0 )
	{
		Graphics_Linux_Shader_Create( &GraphFilterSystemInfoLinux.BrightClip[ UseShader ][ UseFill ][ IsPMA ], GraphicsHardDataLinux.Device.Shader.Base.StretchRect_VS, LINUX_GetFragmentShader( GraphFilterShaderHandle.BrightClipPS[ UseShader ][ UseFill ][ IsPMA ] ) ) ;
	}
	UseAndrShader = &GraphFilterSystemInfoLinux.BrightClip[ UseShader ][ UseFill ][ IsPMA ] ;
	...
	// Uniform の値をセット
	UNIFORM_SET_INT1(   Graphics_Linux_Shader_GetUniformIndex( UseAndrShader, "uSrcTex"         ), 0            ) ;
	UNIFORM_SET_FLOAT4( Graphics_Linux_Shader_GetUniformIndex( UseAndrShader, "uClipBrightness" ), ParamF4[ 0 ] ) ;
	UNIFORM_SET_FLOAT4( Graphics_Linux_Shader_GetUniformIndex( UseAndrShader, "uRGBToY"         ), ParamF4[ 1 ] ) ;
	UNIFORM_SET_FLOAT4( Graphics_Linux_Shader_GetUniformIndex( UseAndrShader, "uClipFillColor"  ), ParamF4[ 2 ] ) ;
...
extern int	GraphFilter_Bright_Scale_PF( GRAPHFILTER_INFO *Info, int BrightMin, int BrightMax, int IsPMA )
{
	...
	DX_LINUX_SHADER_FLOAT4  ParamF4[ 1 ] ;
	GRAPHICS_LINUX_SHADER *UseAndrShader ;

	// 使用するシェーダーのセットアップ
	if( GraphFilterShaderHandle.BrightScalePS[ IsPMA ] < 0 )
	{
		GraphFilterShaderHandle.BrightScalePS[ IsPMA ] = LINUX_MemLoadShaderCode( FlagFileName[ IsPMA ], DX_SHADERTYPE_PIXEL ) ;
		if( GraphFilterShaderHandle.BrightScalePS[ IsPMA ] < 0 )
		{
			char PathUTF16LE[ 128 ] ;
	...
	if( GraphFilterSystemInfoLinux.BrightScale[ IsPMA ].Shader == 0 )
	{
		Graphics_Linux_Shader_Create( &GraphFilterSystemInfoLinux.BrightScale[ IsPMA ], GraphicsHardDataLinux.Device.Shader.Base.StretchRect_VS, LINUX_GetFragmentShader( GraphFilterShaderHandle.BrightScalePS[ IsPMA ] ) ) ;
	}
	UseAndrShader = &GraphFilterSystemInfoLinux.BrightScale[ IsPMA ] ;
	...
	// Uniform の値をセット
	UNIFORM_SET_INT1(   Graphics_Linux_Shader_GetUniformIndex( UseAndrShader, "uSrcTex"      ), 0            ) ;
	UNIFORM_SET_FLOAT4( Graphics_Linux_Shader_GetUniformIndex( UseAndrShader, "uMinB_ScaleB" ), ParamF4[ 0 ] ) ;
...
extern int	GraphFilter_HSB_PF(         GRAPHFILTER_INFO *Info, int HueType, float Hue, float Saturation, float Bright, int IsPMA )
{
	...
	{
		GraphFilterShaderHandle.HsbPS[ UseShader ][ IsPMA ] = LINUX_MemLoadShaderCode( FlagFileName[ UseShader ][ IsPMA ], DX_SHADERTYPE_PIXEL ) ;
		if( GraphFilterShaderHandle.HsbPS[ UseShader ][ IsPMA ] < 0 )
		{
			char PathUTF16LE[ 128 ] ;
	...
	if( GraphFilterSystemInfoLinux.Hsb[ UseShader ][ IsPMA ].Shader == 0 )
	{
		Graphics_Linux_Shader_Create( &GraphFilterSystemInfoLinux.Hsb[ UseShader ][ IsPMA ], GraphicsHardDataLinux.Device.Shader.Base.StretchRect_VS, LINUX_GetFragmentShader( GraphFilterShaderHandle.HsbPS[ UseShader ][ IsPMA ] ) ) ;
	}
	UseAndrShader = &GraphFilterSystemInfoLinux.Hsb[ UseShader ][ IsPMA ] ;
	...
	// Uniform の値をセット
	UNIFORM_SET_INT1(   Graphics_Linux_Shader_GetUniformIndex( UseAndrShader, "uSrcTex" ), 0            ) ;
	UNIFORM_SET_FLOAT4( Graphics_Linux_Shader_GetUniformIndex( UseAndrShader, "uParam"  ), ParamF4[ 0 ] ) ;
...
extern int	GraphFilter_Invert_PF(      GRAPHFILTER_INFO *Info, int IsPMA )
{
	...
	{
		GraphFilterShaderHandle.InvertPS[ IsPMA ] = LINUX_MemLoadShaderCode( FlagFileName[ IsPMA ], DX_SHADERTYPE_PIXEL ) ;
		if( GraphFilterShaderHandle.InvertPS[ IsPMA ] < 0 )
		{
			char PathUTF16LE[ 128 ] ;
	...
	if( GraphFilterSystemInfoLinux.Invert[ IsPMA ].Shader == 0 )
	{
		Graphics_Linux_Shader_Create( &GraphFilterSystemInfoLinux.Invert[ IsPMA ], GraphicsHardDataLinux.Device.Shader.Base.StretchRect_VS, LINUX_GetFragmentShader( GraphFilterShaderHandle.InvertPS[ IsPMA ] ) ) ;
	}
	UseAndrShader = &GraphFilterSystemInfoLinux.Invert[ IsPMA ] ;
...
extern int	GraphFilter_Level_PF(       GRAPHFILTER_INFO *Info, float Min, float Max, float Gamma, float AfterMin, float AfterMax, int IsPMA )
{
	...
	{
		GraphFilterShaderHandle.LevelPS[ IsPMA ] = LINUX_MemLoadShaderCode( FlagFileName[ IsPMA ], DX_SHADERTYPE_PIXEL ) ;
		if( GraphFilterShaderHandle.LevelPS[ IsPMA ] < 0 )
		{
			char PathUTF16LE[ 128 ] ;
	...
	if( GraphFilterSystemInfoLinux.Level[ IsPMA ].Shader == 0 )
	{
		Graphics_Linux_Shader_Create( &GraphFilterSystemInfoLinux.Level[ IsPMA ], GraphicsHardDataLinux.Device.Shader.Base.StretchRect_VS, LINUX_GetFragmentShader( GraphFilterShaderHandle.LevelPS[ IsPMA ] ) ) ;
	}
	UseAndrShader = &GraphFilterSystemInfoLinux.Level[ IsPMA ] ;
	...
	// Uniform の値をセット
	UNIFORM_SET_INT1(   Graphics_Linux_Shader_GetUniformIndex( UseAndrShader, "uSrcTex"                       ), 0            ) ;
	UNIFORM_SET_INT1(   Graphics_Linux_Shader_GetUniformIndex( UseAndrShader, "uGammaTex"                     ), 1            ) ;
	UNIFORM_SET_FLOAT4( Graphics_Linux_Shader_GetUniformIndex( UseAndrShader, "uMin_LenRev_AfterMin_AfterMax" ), ParamF4[ 0 ] ) ;

	LINUX_FilterStretchBlt( UseAndrShader, Info, TRUE, 1, GraphFilterShaderHandle.GammaTex ) ;
...
extern int	GraphFilter_TwoColor_PF(    GRAPHFILTER_INFO *Info, float Threshold, COLOR_F *LowColor, COLOR_F *HighColor, int IsPMA )
{
	...
	{
		GraphFilterShaderHandle.TwoColorPS[ IsPMA ] = LINUX_MemLoadShaderCode( FlagFileName[ IsPMA ], DX_SHADERTYPE_PIXEL ) ;
		if( GraphFilterShaderHandle.TwoColorPS[ IsPMA ] < 0 )
		{
			char PathUTF16LE[ 128 ] ;
	...
	if( GraphFilterSystemInfoLinux.TwoColor[ IsPMA ].Shader == 0 )
	{
		Graphics_Linux_Shader_Create( &GraphFilterSystemInfoLinux.TwoColor[ IsPMA ], GraphicsHardDataLinux.Device.Shader.Base.StretchRect_VS, LINUX_GetFragmentShader( GraphFilterShaderHandle.TwoColorPS[ IsPMA ] ) ) ;
	}
	UseAndrShader = &GraphFilterSystemInfoLinux.TwoColor[ IsPMA ] ;
	...
	// Uniform の値をセット
	UNIFORM_SET_INT1(   Graphics_Linux_Shader_GetUniformIndex( UseAndrShader, "uSrcTex"     ), 0            ) ;
	UNIFORM_SET_FLOAT4( Graphics_Linux_Shader_GetUniformIndex( UseAndrShader, "uRGBToY_Cmp" ), ParamF4[ 0 ] ) ;
	UNIFORM_SET_FLOAT4( Graphics_Linux_Shader_GetUniformIndex( UseAndrShader, "uLowColor"   ), ParamF4[ 1 ] ) ;
	UNIFORM_SET_FLOAT4( Graphics_Linux_Shader_GetUniformIndex( UseAndrShader, "uHighColor"  ), ParamF4[ 2 ] ) ;
...
extern int	GraphFilter_GradientMap_PF( GRAPHFILTER_INFO *Info, int MapGrHandle, int Reverse, int IsPMA )
{
	...
	{
		GraphFilterShaderHandle.GradientMapPS[ UseShader ][ IsPMA ] = LINUX_MemLoadShaderCode( FlagFileName[ UseShader ][ IsPMA ], DX_SHADERTYPE_PIXEL ) ;
		if( GraphFilterShaderHandle.GradientMapPS[ UseShader ][ IsPMA ] < 0 )
		{
			char PathUTF16LE[ 128 ] ;
	...
	if( GraphFilterSystemInfoLinux.GradientMap[ UseShader ][ IsPMA ].Shader == 0 )
	{
		Graphics_Linux_Shader_Create( &GraphFilterSystemInfoLinux.GradientMap[ UseShader ][ IsPMA ], GraphicsHardDataLinux.Device.Shader.Base.StretchRect_VS, LINUX_GetFragmentShader( GraphFilterShaderHandle.GradientMapPS[ UseShader ][ IsPMA ] ) ) ;
	}
	UseAndrShader = &GraphFilterSystemInfoLinux.GradientMap[ UseShader ][ IsPMA ] ;
	...
	// Uniform の値をセット
	UNIFORM_SET_INT1(   Graphics_Linux_Shader_GetUniformIndex( UseAndrShader, "uSrcTex"     ), 0            ) ;
	UNIFORM_SET_INT1(   Graphics_Linux_Shader_GetUniformIndex( UseAndrShader, "uGradMapTex" ), 1            ) ;
	UNIFORM_SET_FLOAT4( Graphics_Linux_Shader_GetUniformIndex( UseAndrShader, "uRGBToY"     ), ParamF4[ 0 ] ) ;

	LINUX_FilterStretchBlt( UseAndrShader, Info, FALSE, 1, MapGrHandle ) ;
...
extern int	GraphFilter_Replacement_PF( GRAPHFILTER_INFO *Info, COLOR_U8 TargetColor, COLOR_U8 NextColor, int IsPMA )
{
	...
	{
		GraphFilterShaderHandle.ReplacementPS[ IsPMA ] = LINUX_MemLoadShaderCode( FlagFileName[ IsPMA ], DX_SHADERTYPE_PIXEL ) ;
		if( GraphFilterShaderHandle.ReplacementPS[ IsPMA ] < 0 )
		{
			char PathUTF16LE[ 128 ] ;
	...
	if( GraphFilterSystemInfoLinux.Replacement[ IsPMA ].Shader == 0 )
	{
		Graphics_Linux_Shader_Create( &GraphFilterSystemInfoLinux.Replacement[ IsPMA ], GraphicsHardDataLinux.Device.Shader.Base.StretchRect_VS, LINUX_GetFragmentShader( GraphFilterShaderHandle.ReplacementPS[ IsPMA ] ) ) ;
	}
	UseAndrShader = &GraphFilterSystemInfoLinux.Replacement[ IsPMA ] ;
	...
	// Uniform の値をセット
	UNIFORM_SET_INT1(   Graphics_Linux_Shader_GetUniformIndex( UseAndrShader, "uSrcTex"      ), 0            ) ;
	UNIFORM_SET_FLOAT4( Graphics_Linux_Shader_GetUniformIndex( UseAndrShader, "uTargetColor" ), ParamF4[ 0 ] ) ;
	UNIFORM_SET_FLOAT4( Graphics_Linux_Shader_GetUniformIndex( UseAndrShader, "uNextColor"   ), ParamF4[ 1 ] ) ;
...
extern int	GraphFilter_PremulAlpha_PF( GRAPHFILTER_INFO *Info )
{
	...
	{
		GraphFilterShaderHandle.PreMulAlphaPS = LINUX_MemLoadShaderCode( FlagFileName[ UseShader ], DX_SHADERTYPE_PIXEL ) ;
		if( GraphFilterShaderHandle.PreMulAlphaPS < 0 )
		{
			char PathUTF16LE[ 128 ] ;
	...
	if( GraphFilterSystemInfoLinux.PreMulAlpha.Shader == 0 )
	{
		Graphics_Linux_Shader_Create( &GraphFilterSystemInfoLinux.PreMulAlpha, GraphicsHardDataLinux.Device.Shader.Base.StretchRect_VS, LINUX_GetFragmentShader( GraphFilterShaderHandle.PreMulAlphaPS ) ) ;
	}
	UseAndrShader = &GraphFilterSystemInfoLinux.PreMulAlpha ;
...
extern int	GraphFilter_InterpAlpha_PF( GRAPHFILTER_INFO *Info )
{
	...
	{
		GraphFilterShaderHandle.InterpAlphaPS = LINUX_MemLoadShaderCode( FlagFileName[ UseShader ], DX_SHADERTYPE_PIXEL ) ;
		if( GraphFilterShaderHandle.InterpAlphaPS < 0 )
		{
			char PathUTF16LE[ 128 ] ;
	...
	if( GraphFilterSystemInfoLinux.InterpAlpha.Shader == 0 )
	{
		Graphics_Linux_Shader_Create( &GraphFilterSystemInfoLinux.InterpAlpha, GraphicsHardDataLinux.Device.Shader.Base.StretchRect_VS, LINUX_GetFragmentShader( GraphFilterShaderHandle.InterpAlphaPS ) ) ;
	}
	UseAndrShader = &GraphFilterSystemInfoLinux.InterpAlpha ;
...
extern int	GraphFilter_YUVtoRGB_PF( GRAPHFILTER_INFO *Info, int UVGrHandle )
{
	...
	{
		GraphFilterShaderHandle.YUVtoRGBPS[ UseShader ] = LINUX_MemLoadShaderCode( FlagFileName[ UseShader ], DX_SHADERTYPE_PIXEL ) ;
		if( GraphFilterShaderHandle.YUVtoRGBPS[ UseShader ] < 0 )
		{
			char PathUTF16LE[ 128 ] ;
	...
	if( GraphFilterSystemInfoLinux.YUVtoRGB[ UseShader ].Shader == 0 )
	{
		Graphics_Linux_Shader_Create( &GraphFilterSystemInfoLinux.YUVtoRGB[ UseShader ], GraphicsHardDataLinux.Device.Shader.Base.StretchRect_VS, LINUX_GetFragmentShader( GraphFilterShaderHandle.YUVtoRGBPS[ UseShader ] ) ) ;
	}
	UseAndrShader = &GraphFilterSystemInfoLinux.YUVtoRGB[ UseShader ] ;
	...
	// Uniform の値をセット
	UNIFORM_SET_INT1(   Graphics_Linux_Shader_GetUniformIndex( UseAndrShader, "uSrcTex"                 ), 0            ) ;
	UNIFORM_SET_INT1(   Graphics_Linux_Shader_GetUniformIndex( UseAndrShader, "uUVTex"                  ), 1            ) ;
	UNIFORM_SET_FLOAT4( Graphics_Linux_Shader_GetUniformIndex( UseAndrShader, "uUVTexUVScale_RRA_Add_U" ), ParamF4[ 0 ] ) ;

	LINUX_FilterStretchBlt( UseAndrShader, Info, FALSE, 1, UVGrHandle ) ;
...
extern int	GraphFilter_BicubicScale_PF( GRAPHFILTER_INFO *Info, int DestSizeX, int DestSizeY )
{
	...
	{
		GraphFilterShaderHandle.BicubicPS = LINUX_MemLoadShaderCode( PsoFileName[ UseShader ], DX_SHADERTYPE_PIXEL ) ;
		if( GraphFilterShaderHandle.BicubicPS < 0 )
		{
			char PathUTF16LE[ 128 ] ;
	...
	if( GraphFilterSystemInfoLinux.Bicubic.Shader == 0 )
	{
		Graphics_Linux_Shader_Create( &GraphFilterSystemInfoLinux.Bicubic, GraphicsHardDataLinux.Device.Shader.Base.StretchRectTex8_VS, LINUX_GetFragmentShader( GraphFilterShaderHandle.BicubicPS ) ) ;
	}
	UseAndrShader = &GraphFilterSystemInfoLinux.Bicubic ;
	...
	// Uniform の値をセット
	UNIFORM_SET_INT1(   Graphics_Linux_Shader_GetUniformIndex( UseAndrShader, "uSrcTex"           ), 0            ) ;
	UNIFORM_SET_FLOAT4( Graphics_Linux_Shader_GetUniformIndex( UseAndrShader, "uTexSize_X2PixelU" ), ParamF4[ 0 ] ) ;

	LINUX_FilterStretchBlt( UseAndrShader, Info, FALSE, 1, -1, VertexTex8 ) ;
...
extern int	GraphFilter_Lanczos3Scale_PF( GRAPHFILTER_INFO *Info, int DestSizeX, int DestSizeY )
{
	...
	{
		GraphFilterShaderHandle.Lanczos3PS = LINUX_MemLoadShaderCode( PsoFileName[ UseShader ], DX_SHADERTYPE_PIXEL ) ;
		if( GraphFilterShaderHandle.Lanczos3PS < 0 )
		{
			char PathUTF16LE[ 128 ] ;
	...
	if( GraphFilterSystemInfoLinux.Lanczos3.Shader == 0 )
	{
		Graphics_Linux_Shader_Create( &GraphFilterSystemInfoLinux.Lanczos3, GraphicsHardDataLinux.Device.Shader.Base.StretchRectTex8_VS, LINUX_GetFragmentShader( GraphFilterShaderHandle.Lanczos3PS ) ) ;
	}
	UseAndrShader = &GraphFilterSystemInfoLinux.Lanczos3 ;
	...
	// Uniform の値をセット
	UNIFORM_SET_INT1(   Graphics_Linux_Shader_GetUniformIndex( UseAndrShader, "uSrcTex"           ), 0            ) ;
	UNIFORM_SET_FLOAT4( Graphics_Linux_Shader_GetUniformIndex( UseAndrShader, "uTexSize_X1PixelU" ), ParamF4[ 0 ] ) ;
...
extern int GraphFilter_DestGraphSetup_PF( GRAPHFILTER_INFO *Info, int *UseSrcGrHandle, int *UseDestGrHandle )
{
	...
	if( SrcEqDestOrDestNonDrawValidHandle || AlwaysUseDestWorkHandle )
	{
		...
		*UseDestGrHandle = LINUX_GraphFilter_GetWorkTexture( DestTexFloatType, ( DWORD )TexSizeW, ( DWORD )TexSizeH, ( DWORD )UseDestWorkHandleIndex ) ;
	...
	if( SrcGraphDivFlag )
	{
		...
		*UseSrcGrHandle = LINUX_GraphFilter_GetWorkTexture( SrcTexFloatType, ( DWORD )TexSizeW, ( DWORD )TexSizeH, ( DWORD )UseSrcWorkHandleIndex ) ;
		if( *UseSrcGrHandle < 0 )
		{
			return -1 ;
		}
...
extern int	GraphBlend_Basic_PF(           GRAPHFILTER_INFO *Info, int IsPMA )
{
	...
	{
		GraphFilterShaderHandle.BasicBlendPS[ UseShader ] = LINUX_MemLoadShaderCode( FlagFileName[ UseShader ], DX_SHADERTYPE_PIXEL ) ;
		if( GraphFilterShaderHandle.BasicBlendPS[ UseShader ] < 0 )
		{
			char PathUTF16LE[ 128 ] ;
	...
	if( GraphFilterSystemInfoLinux.BasicBlend[ UseShader ].Shader == 0 )
	{
		Graphics_Linux_Shader_Create( &GraphFilterSystemInfoLinux.BasicBlend[ UseShader ], GraphicsHardDataLinux.Device.Shader.Base.StretchRectTex2_VS, LINUX_GetFragmentShader( GraphFilterShaderHandle.BasicBlendPS[ UseShader ] ) ) ;
	}
	UseAndrShader = &GraphFilterSystemInfoLinux.BasicBlend[ UseShader ] ;
	...
	// Uniform の値をセット
	UNIFORM_SET_INT1(   Graphics_Linux_Shader_GetUniformIndex( UseAndrShader, "uSrcTex"     ), 0            ) ;
	UNIFORM_SET_INT1(   Graphics_Linux_Shader_GetUniformIndex( UseAndrShader, "uBlendTex"   ), 1            ) ;
	UNIFORM_SET_FLOAT4( Graphics_Linux_Shader_GetUniformIndex( UseAndrShader, "uBlendRatio" ), ParamF4[ 0 ] ) ;

	LINUX_FilterStretchBlt( UseAndrShader, Info, Info->BlendGraphScalingFilterIsBilinear ) ;
...
extern int	GraphBlend_RGBA_Select_Mix_PF( GRAPHFILTER_INFO *Info, int SelectR, int SelectG, int SelectB, int SelectA, int IsPMA )
{
	...
	if( SrcOnlyFlag )
	{
		...
		{
			if( GLINUX.ShaderCode.Base.RGBAMixS_ShaderPackImage != NULL )
			{
				*PixelShaderHandle = NS_LoadPixelShaderFromMem( GLINUX.ShaderCode.Base.RGBAMixS_FS_Code[ SelectR ][ SelectG ][ SelectB ][ SelectA ][ IsPMA ].Binary, GLINUX.ShaderCode.Base.RGBAMixS_FS_Code[ SelectR ][ SelectG ][ SelectB ][ SelectA ][ IsPMA ].Size ) ;
				if( *PixelShaderHandle < 0 )
				{
		...
		if( GraphFilterSystemInfoLinux.RgbaMixS[ SelectR ][ SelectG ][ SelectB ][ SelectA ][ IsPMA ].Shader == 0 )
		{
			Graphics_Linux_Shader_Create( &GraphFilterSystemInfoLinux.RgbaMixS[ SelectR ][ SelectG ][ SelectB ][ SelectA ][ IsPMA ], GraphicsHardDataLinux.Device.Shader.Base.StretchRectTex2_VS, LINUX_GetFragmentShader( *PixelShaderHandle ) ) ;
		}
		UseAndrShader = &GraphFilterSystemInfoLinux.RgbaMixS[ SelectR ][ SelectG ][ SelectB ][ SelectA ][ IsPMA ] ;
	}
	else
	if( SelectR == SelectG && SelectR == SelectB && SelectR != DX_RGBA_SELECT_BLEND_A && SelectR != DX_RGBA_SELECT_SRC_A )
	{
		...
		if( *PixelShaderHandle < 0 )
		{
			...
			*PixelShaderHandle = LINUX_MemLoadShaderCode( FileName, DX_SHADERTYPE_PIXEL ) ;
			if( *PixelShaderHandle < 0 )
			{
		...
		if( GraphFilterSystemInfoLinux.RgbaMixSRRRB[ SelectR ][ SelectA - DX_RGBA_SELECT_BLEND_R ][ IsPMA ].Shader == 0 )
		{
			Graphics_Linux_Shader_Create( &GraphFilterSystemInfoLinux.RgbaMixSRRRB[ SelectR ][ SelectA - DX_RGBA_SELECT_BLEND_R ][ IsPMA ], GraphicsHardDataLinux.Device.Shader.Base.StretchRectTex2_VS, LINUX_GetFragmentShader( *PixelShaderHandle ) ) ;
		}
		UseAndrShader = &GraphFilterSystemInfoLinux.RgbaMixSRRRB[ SelectR ][ SelectA - DX_RGBA_SELECT_BLEND_R ][ IsPMA ] ;
	}
	else
		...
		( SelectR == DX_RGBA_SELECT_BLEND_R && SelectG == DX_RGBA_SELECT_BLEND_G && SelectB == DX_RGBA_SELECT_BLEND_B ) )
	{
		...
		if( GraphFilterSystemInfoLinux.RgbaMixSRGBB[ SelectA - DX_RGBA_SELECT_BLEND_R ][ IsPMA ].Shader == 0 )
		{
			Graphics_Linux_Shader_Create( &GraphFilterSystemInfoLinux.RgbaMixSRGBB[ SelectA - DX_RGBA_SELECT_BLEND_R ][ IsPMA ], GraphicsHardDataLinux.Device.Shader.Base.StretchRectTex2_VS, LINUX_GetFragmentShader( *PixelShaderHandle ) ) ;
		}
		UseAndrShader = &GraphFilterSystemInfoLinux.RgbaMixSRGBB[ SelectA - DX_RGBA_SELECT_BLEND_R ][ IsPMA ] ;
	}
	else
	{
USE_BASE_SHADER:
		...
		if( *PixelShaderHandle < 0 )
		{
			*PixelShaderHandle = LINUX_MemLoadShaderCode( FlagFileName[ IsPMA ], DX_SHADERTYPE_PIXEL ) ;
			if( *PixelShaderHandle < 0 )
			{
				ConvString( FlagFileName[ IsPMA ], -1, DX_CHARCODEFORMAT_ASCII, ( char * )PathUTF16LE, sizeof( PathUTF16LE ), DX_CHARCODEFORMAT_UTF16LE ) ;
				DXST_LOGFILEFMT_ADDUTF16LE(( "\xd5\x30\xa3\x30\xeb\x30\xbf\x30\xfc\x30\x28\x75\xb7\x30\xa7\x30\xfc\x30\xc0\x30\xfc\x30\x6e\x30\x5c\x4f\x10\x62\x6b\x30\x31\x59\x57\x65\x57\x30\x7e\x30\x57\x30\x5f\x30\x20\x00\x25\x00\x73\x00\x00"/*@ L"フィルター用シェーダーの作成に失敗しました %s" @*/, PathUTF16LE )) ;
		...
		if( GraphFilterSystemInfoLinux.RgbaMixBase[ IsPMA ].Shader == 0 )
		{
			Graphics_Linux_Shader_Create( &GraphFilterSystemInfoLinux.RgbaMixBase[ IsPMA ], GraphicsHardDataLinux.Device.Shader.Base.StretchRectTex2_VS, LINUX_GetFragmentShader( *PixelShaderHandle ) ) ;
		}
		UseAndrShader = &GraphFilterSystemInfoLinux.RgbaMixBase[ IsPMA ] ;
	}
	...
	UNIFORM_SET_INT1(   Graphics_Linux_Shader_GetUniformIndex( UseAndrShader, "uSrcTex"     ), 0            ) ;
	UNIFORM_SET_INT1(   Graphics_Linux_Shader_GetUniformIndex( UseAndrShader, "uBlendTex"   ), 1            ) ;
	UNIFORM_SET_FLOAT4( Graphics_Linux_Shader_GetUniformIndex( UseAndrShader, "uRGBASelect" ), ParamF4[ 0 ] ) ;
...
extern int GraphFilter_Linux_ReleaseShaderAll( void )
{
	Graphics_Linux_ShaderArray_Release( ( GRAPHICS_LINUX_SHADER * )GraphFilterSystemInfoLinux.Gauss,			sizeof( GraphFilterSystemInfoLinux.Gauss			) / sizeof( GRAPHICS_LINUX_SHADER ) ) ;
	Graphics_Linux_ShaderArray_Release( ( GRAPHICS_LINUX_SHADER * )GraphFilterSystemInfoLinux.BrightClip,		sizeof( GraphFilterSystemInfoLinux.BrightClip		) / sizeof( GRAPHICS_LINUX_SHADER ) ) ;
	Graphics_Linux_ShaderArray_Release( ( GRAPHICS_LINUX_SHADER * )GraphFilterSystemInfoLinux.BrightScale,	sizeof( GraphFilterSystemInfoLinux.BrightScale	) / sizeof( GRAPHICS_LINUX_SHADER ) ) ;
	Graphics_Linux_ShaderArray_Release( ( GRAPHICS_LINUX_SHADER * )GraphFilterSystemInfoLinux.DownScale,		sizeof( GraphFilterSystemInfoLinux.DownScale		) / sizeof( GRAPHICS_LINUX_SHADER ) ) ;
	Graphics_Linux_ShaderArray_Release( ( GRAPHICS_LINUX_SHADER * )GraphFilterSystemInfoLinux.Hsb,			sizeof( GraphFilterSystemInfoLinux.Hsb			) / sizeof( GRAPHICS_LINUX_SHADER ) ) ;
	Graphics_Linux_ShaderArray_Release( ( GRAPHICS_LINUX_SHADER * )&GraphFilterSystemInfoLinux.Mono,			sizeof( GraphFilterSystemInfoLinux.Mono			) / sizeof( GRAPHICS_LINUX_SHADER ) ) ;
	Graphics_Linux_ShaderArray_Release( ( GRAPHICS_LINUX_SHADER * )GraphFilterSystemInfoLinux.Invert,			sizeof( GraphFilterSystemInfoLinux.Invert			) / sizeof( GRAPHICS_LINUX_SHADER ) ) ;
	Graphics_Linux_ShaderArray_Release( ( GRAPHICS_LINUX_SHADER * )GraphFilterSystemInfoLinux.Level,			sizeof( GraphFilterSystemInfoLinux.Level			) / sizeof( GRAPHICS_LINUX_SHADER ) ) ;
	Graphics_Linux_ShaderArray_Release( ( GRAPHICS_LINUX_SHADER * )GraphFilterSystemInfoLinux.TwoColor,		sizeof( GraphFilterSystemInfoLinux.TwoColor		) / sizeof( GRAPHICS_LINUX_SHADER ) ) ;
	Graphics_Linux_ShaderArray_Release( ( GRAPHICS_LINUX_SHADER * )GraphFilterSystemInfoLinux.Replacement,	sizeof( GraphFilterSystemInfoLinux.Replacement	) / sizeof( GRAPHICS_LINUX_SHADER ) ) ;
	Graphics_Linux_ShaderArray_Release( ( GRAPHICS_LINUX_SHADER * )GraphFilterSystemInfoLinux.GradientMap,	sizeof( GraphFilterSystemInfoLinux.GradientMap	) / sizeof( GRAPHICS_LINUX_SHADER ) ) ;
	Graphics_Linux_ShaderArray_Release( ( GRAPHICS_LINUX_SHADER * )&GraphFilterSystemInfoLinux.PreMulAlpha,	sizeof( GraphFilterSystemInfoLinux.PreMulAlpha	) / sizeof( GRAPHICS_LINUX_SHADER ) ) ;
	Graphics_Linux_ShaderArray_Release( ( GRAPHICS_LINUX_SHADER * )&GraphFilterSystemInfoLinux.InterpAlpha,	sizeof( GraphFilterSystemInfoLinux.InterpAlpha	) / sizeof( GRAPHICS_LINUX_SHADER ) ) ;
	Graphics_Linux_ShaderArray_Release( ( GRAPHICS_LINUX_SHADER * )GraphFilterSystemInfoLinux.YUVtoRGB,		sizeof( GraphFilterSystemInfoLinux.YUVtoRGB		) / sizeof( GRAPHICS_LINUX_SHADER ) ) ;
	Graphics_Linux_ShaderArray_Release( ( GRAPHICS_LINUX_SHADER * )&GraphFilterSystemInfoLinux.Bicubic,		sizeof( GraphFilterSystemInfoLinux.Bicubic		) / sizeof( GRAPHICS_LINUX_SHADER ) ) ;
	Graphics_Linux_ShaderArray_Release( ( GRAPHICS_LINUX_SHADER * )&GraphFilterSystemInfoLinux.Lanczos3,		sizeof( GraphFilterSystemInfoLinux.Lanczos3		) / sizeof( GRAPHICS_LINUX_SHADER ) ) ;
	Graphics_Linux_ShaderArray_Release( ( GRAPHICS_LINUX_SHADER * )GraphFilterSystemInfoLinux.BasicBlend,		sizeof( GraphFilterSystemInfoLinux.BasicBlend		) / sizeof( GRAPHICS_LINUX_SHADER ) ) ;
	Graphics_Linux_ShaderArray_Release( ( GRAPHICS_LINUX_SHADER * )GraphFilterSystemInfoLinux.RgbaMixBase,	sizeof( GraphFilterSystemInfoLinux.RgbaMixBase	) / sizeof( GRAPHICS_LINUX_SHADER ) ) ;
	Graphics_Linux_ShaderArray_Release( ( GRAPHICS_LINUX_SHADER * )GraphFilterSystemInfoLinux.RgbaMixSRGBB,	sizeof( GraphFilterSystemInfoLinux.RgbaMixSRGBB	) / sizeof( GRAPHICS_LINUX_SHADER ) ) ;
	Graphics_Linux_ShaderArray_Release( ( GRAPHICS_LINUX_SHADER * )GraphFilterSystemInfoLinux.RgbaMixSRRRB,	sizeof( GraphFilterSystemInfoLinux.RgbaMixSRRRB	) / sizeof( GRAPHICS_LINUX_SHADER ) ) ;
	Graphics_Linux_ShaderArray_Release( ( GRAPHICS_LINUX_SHADER * )GraphFilterSystemInfoLinux.RgbaMixS,		sizeof( GraphFilterSystemInfoLinux.RgbaMixS		) / sizeof( GRAPHICS_LINUX_SHADER ) ) ;
