...
// Linux のヒープ用のメモリ領域を確保するためのコールバック関数群の AutoAlloc_CreateHeapCallback
extern int NormalMemory_AutoAlloc_CreateHeapCallback_PF( int Param, void *Buffer, ALLOCMEM_SIZE_TYPE Size )
{
	PLATFORMHEAP_LINUX *			PlatformHeap = ( PLATFORMHEAP_LINUX * )Buffer ;

	// バッファサイズチェック
	if( sizeof( PLATFORMHEAP_LINUX ) > ALLOCHEAP_CALLBACK_INFO_SIZE )
	{
