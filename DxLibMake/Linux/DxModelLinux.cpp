...
#define MV1_LINUX_COMMONBUFFER_ADDSIZE			(16 * 1024)
...
VERTEXBUFFER_INPUT_INFO_LINUX g_Model_VertexFormat[ 2 ][ 3 ] =
{
	// バンプマップなし
	{
		// 剛体メッシュ
		{
			...
			6,
			{
				...
				{ LINUX_VERTEX_ATTR_TEXUV0_F4,    NULL, 32 },
				{ LINUX_VERTEX_ATTR_TEXUV1_F4,    NULL, 48 },
			},
		},
		...
		// 4ボーン内スキニングメッシュ
		{
			...
			8,
			{
				...
				{ LINUX_VERTEX_ATTR_TEXUV0_F4,    NULL, 32 },
				{ LINUX_VERTEX_ATTR_TEXUV1_F4,    NULL, 48 },
				{ LINUX_VERTEX_ATTR_BLENDIND0,    NULL, 64 },
				{ LINUX_VERTEX_ATTR_BLENDWEIGHT0, NULL, 80 },
			},
		},
		...
		// 8ボーン内スキニングメッシュ
		{
			...
			10,
			{
				...
				{ LINUX_VERTEX_ATTR_BLENDIND0,    NULL, 64 },
				{ LINUX_VERTEX_ATTR_BLENDWEIGHT0, NULL, 80 },
				{ LINUX_VERTEX_ATTR_BLENDIND1,    NULL, 96 },
				{ LINUX_VERTEX_ATTR_BLENDWEIGHT1, NULL, 112 },
			},
		},
	},
	...
	// バンプマップあり
	{
		// 剛体メッシュ
		{
			...
			8,
			{
				...
				{ LINUX_VERTEX_ATTR_TEXUV0_F4,    NULL, 32 },
				{ LINUX_VERTEX_ATTR_TEXUV1_F4,    NULL, 48 },
				{ LINUX_VERTEX_ATTR_TAN,          NULL, 64 },
				{ LINUX_VERTEX_ATTR_BIN,          NULL, 76 },
			},
		},
		...
		// 4ボーン内スキニングメッシュ
		{
			...
			10,
			{
				...
				{ LINUX_VERTEX_ATTR_TAN,          NULL, 64 },
				{ LINUX_VERTEX_ATTR_BIN,          NULL, 76 },
				{ LINUX_VERTEX_ATTR_BLENDIND0,    NULL, 88 },
				{ LINUX_VERTEX_ATTR_BLENDWEIGHT0, NULL, 104 },
			},
		},
		...
		// 8ボーン内スキニングメッシュ
		{
			...
			12,
			{
				...
				{ LINUX_VERTEX_ATTR_BLENDIND0,    NULL,  88 },
				{ LINUX_VERTEX_ATTR_BLENDWEIGHT0, NULL, 104 },
				{ LINUX_VERTEX_ATTR_BLENDIND1,    NULL, 120 },
				{ LINUX_VERTEX_ATTR_BLENDWEIGHT1, NULL, 136 },
			},
		},
	}
} ;

MV1_MODEL_MANAGE_LINUX MV1Man_Linux ;
...
// 指定サイズの汎用バッファの準備を行う
extern int MV1_LINUX_CommonBuffer_Setup( int Size )
{
	if( Size <= MV1Man_Linux.CommonBufferSize )
	{
		return 0 ;
	}

	MV1_LINUX_CommonBuffer_Terminate() ;

	Size += MV1_LINUX_COMMONBUFFER_ADDSIZE ;
	Size = ( Size + 15 ) / 16 * 16 ;

	MV1Man_Linux.CommonBuffer = DXALLOC( ( size_t )Size ) ;
	if( MV1Man_Linux.CommonBuffer == NULL )
	{
	...
	MV1Man_Linux.CommonBufferSize = Size ;
...
// 汎用バッファの後始末を行う
extern int MV1_LINUX_CommonBuffer_Terminate( void )
{
	if( MV1Man_Linux.CommonBuffer == NULL )
	{
		return 0 ;
	...
	DXFREE( MV1Man_Linux.CommonBuffer ) ;
	MV1Man_Linux.CommonBuffer = NULL ;

	MV1Man_Linux.CommonBufferSize = 0 ;
...
extern void MV1_SetupPackDrawInfo_PF( MV1_MODEL_BASE *ModelBase )
{
	...
	MaxPackDrawNum = DX_LINUX_VS_CONST_WORLD_MAT_NUM / ModelBase->TriangleListMinMatrixNum ;
	if( ModelBase->PackDrawMaxNum > MaxPackDrawNum )
	{
		ModelBase->PackDrawMaxNum = MaxPackDrawNum ;
	}
	...
	for( i = 0 ; i < ModelBase->TriangleListNum ; i ++, MBTList ++ )
	{
		...
		MaxPackDrawNum = DX_LINUX_VS_CONST_WORLD_MAT_NUM / UseBoneNum ;
		if( MBTList->PackDrawMaxNum > MaxPackDrawNum )
		{
			MBTList->PackDrawMaxNum = MaxPackDrawNum ;
		}
...
// MV1_LINUX_SetupVertexBufferBase_PF の頂点データ作成用の補助関数
__inline void MV1_LINUX_SetVertexData(
	      BYTE **		DestP,
...
// 頂点データのサイズを取得する
static DWORD MV1_LINUX_GetVertexSize( int Bump, int Type /* 0:剛体  1:4ボーン  2:8ボーン */  )
{
	DWORD Result ;

	Result = 
		sizeof( VECTOR ) +				//	{ "POSITION",     0, D_DXGI_FORMAT_R32G32B32_FLOAT,    0,  0, D_LINUX_INPUT_PER_VERTEX_DATA, 0 },
		sizeof( VECTOR ) +				//	{ "NORMAL",       0, D_DXGI_FORMAT_R32G32B32_FLOAT,    0, 12, D_LINUX_INPUT_PER_VERTEX_DATA, 0 },
		sizeof( COLOR_U8 ) +			//	{ "COLOR",        0, D_DXGI_FORMAT_B8G8R8A8_UNORM,     0, 24, D_LINUX_INPUT_PER_VERTEX_DATA, 0 },
		sizeof( COLOR_U8 ) +			//	{ "COLOR",        1, D_DXGI_FORMAT_B8G8R8A8_UNORM,     0, 28, D_LINUX_INPUT_PER_VERTEX_DATA, 0 },
		sizeof( FLOAT4 ) +				//	{ "TEXCOORD",     0, D_DXGI_FORMAT_R32G32B32A32_FLOAT, 0, 32, D_LINUX_INPUT_PER_VERTEX_DATA, 0 },
		sizeof( FLOAT4 ) ;				//	{ "TEXCOORD",     1, D_DXGI_FORMAT_R32G32B32A32_FLOAT, 0, 48, D_LINUX_INPUT_PER_VERTEX_DATA, 0 },

	if( Bump != 0 )
	{
		Result +=
			sizeof( VECTOR ) +			//	MakeLINUXInputElement( "TANGENT",      0, D_DXGI_FORMAT_R32G32B32_FLOAT,    0, offset ) ;
			sizeof( VECTOR ) ;			//	MakeLINUXInputElement( "BINORMAL",     0, D_DXGI_FORMAT_R32G32B32_FLOAT,    0, offset ) ;
	}
	...
	case 2 :	// ８ボーン以内スキニングメッシュ
		Result +=
			sizeof( FLOAT4 ) +			//	MakeLINUXInputElement( "BLENDINDICES", 0, D_DXGI_FORMAT_R32G32B32A32_FLOAT, 0, offset ) ;
			sizeof( FLOAT4 ) ;			//	MakeLINUXInputElement( "BLENDWEIGHT",  0, D_DXGI_FORMAT_R32G32B32A32_FLOAT, 0, offset ) ;

		// ８ボーン以内スキニングメッシュ
		if( Type == 2 )
		{
			Result +=
				sizeof( FLOAT4 ) +		//	MakeLINUXInputElement( "BLENDINDICES", 1, D_DXGI_FORMAT_R32G32B32A32_FLOAT, 0, offset ) ;
				sizeof( FLOAT4 ) ;		//	MakeLINUXInputElement( "BLENDWEIGHT",  1, D_DXGI_FORMAT_R32G32B32A32_FLOAT, 0, offset ) ;
		}
		break ;
	}
	...
	for( i = 0 ; i < 3 ; i ++ )
	{
		for( j = 0 ; j < 2 ; j ++ )
		{
			for( k = 0 ; k < 9 ; k ++ )
			{
				...
				{
					// １頂点あたりのサイズを取得
					TVBuf->UnitSize = MV1_LINUX_GetVertexSize( j, i ) ;
		...
		for( l = 0 ; l < MBFrame->MeshNum ; l ++, MBMesh ++, Mesh ++ )
		{
			...
			for( m = 0 ; m < MBMesh->TriangleListNum ; m ++, MBTList ++, TList = ( MV1_TRIANGLE_LIST * )( ( BYTE * )TList + sizeof( MV1_TRIANGLE_LIST ) + sizeof( MV1_TRIANGLE_LIST_PF ) ) )
			{
				...
				else
				{
					...
					// テンポラリバッファの準備
					if( MV1_LINUX_CommonBuffer_Setup( ( int )( MBTList->VertexBuffer->UnitSize * MBTList->VertexNum ) ) < 0 )
					{
						continue ;
					}

					Dest = ( BYTE * )MV1Man_Linux.CommonBuffer ;
				}
...
//				DestUnitSize = MBTList->VertexBuffer->UnitSize ;
				...
				if( MBTList->VertexType != MV1_VERTEX_TYPE_SKIN_FREEBONE )
				{
					...
					glBufferSubData( GL_ARRAY_BUFFER, 0, MBTList->VertexBuffer->UnitSize * MBTList->VertexNum, MV1Man_Linux.CommonBuffer ) ;
				}
			}
		}
	}
...
extern int MV1_BeginRender_PF( MV1_MODEL *Model )
{
	...
	// Graphics_Linux_RenderBegin を実行していなかったら実行する
	if( GLINUX.Device.DrawInfo.BeginSceneFlag == FALSE )
	{
		Graphics_Linux_RenderBegin() ;
	}
	...
	// 深度バッファ設定を反映する
	Graphics_Linux_DeviceState_SetDepthEnable(      Model->EnableZBufferFlag ) ;
	Graphics_Linux_DeviceState_SetDepthWriteEnable( Model->WriteZBufferFlag ) ;
	Graphics_Linux_DeviceState_SetDepthFunc(        g_DXCmpModeToGLES2CompareFunc[ Model->ZBufferCmpType ] ) ;
	Graphics_Linux_DeviceState_SetDepthBias(        Model->ZBias ) ;
	...
	for( i = 0 ; i < 3 ; i ++ )
	{
		Graphics_Linux_DeviceState_SetMaxAnisotropy( Model->MaxAnisotropy, i ) ;
	}
	...
	GLINUX.Device.State.DrawMode                 = DX_DRAWMODE_OTHER ;
	GLINUX.Device.DrawSetting.AlphaTestMode      = -1 ;
	GLINUX.Device.DrawSetting.AlphaTestParam     = 0 ;
	GLINUX.Device.DrawSetting.DrawPrepAlwaysFlag = TRUE ;
	GLINUX.Device.DrawSetting.ChangeTextureFlag  = TRUE ;

	// カラーにアルファ値を乗算するかどうかの情報をセットする
	Graphics_Linux_DeviceState_SetMulAlphaColor( Model->UseDrawMulAlphaColor ) ;
...
{
	Graphics_Linux_DeviceState_SetWorldMatrix( &GSYS.DrawSetting.WorldMatrixF ) ;
...
static bool MV1_LINUX_SetupMeshDrawMaterialCommon( MV1_MESH *Mesh )
{
	MV1_MESH_BASE     *MBMesh ;
...
//	MBMaterial = MMaterial->BaseData ;
	...
	GLINUX.Device.DrawSetting.DrawPrepAlwaysFlag = TRUE ;
	Graphics_Linux_DeviceState_SetMaterial( &Material ) ;

	// カリング設定
	Graphics_Linux_DeviceState_SetCullMode( GSYS.DrawSetting.ShadowMapDraw ? FALSE : MBMesh->BackCulling ) ; 

	// アルファテスト設定
//	if( MMaterial->UseAlphaTest )
//	{
//		Graphics_Linux_DeviceState_SetAlphaTestCmpMode( MMaterial->AlphaFunc ) ;
//		Graphics_Linux_DeviceState_SetAlphaTestRef(     MMaterial->AlphaRef ) ;
//	}
	...
	case DX_BLENDMODE_NOBLEND :
		Graphics_Linux_DeviceState_SetBlendMode( BlendMode, FALSE, DX_BLEND_ONE, DX_BLEND_ZERO, DX_BLENDOP_ADD, DX_BLEND_ONE, DX_BLEND_ZERO, DX_BLENDOP_ADD, GSYS.DrawSetting.NotWriteAlphaChannelFlag ) ;
		break ;

	case DX_BLENDMODE_ALPHA :
		if( Frame->Container->UseDrawMulAlphaColor )
		{
			Graphics_Linux_DeviceState_SetBlendMode( DX_BLENDMODE_PMA_ALPHA, FALSE, DX_BLEND_ONE, DX_BLEND_ZERO, DX_BLENDOP_ADD, DX_BLEND_ONE, DX_BLEND_ZERO, DX_BLENDOP_ADD, GSYS.DrawSetting.NotWriteAlphaChannelFlag ) ;
		}
		else
		{
			Graphics_Linux_DeviceState_SetBlendMode( DX_BLENDMODE_ALPHA, FALSE, DX_BLEND_ONE, DX_BLEND_ZERO, DX_BLENDOP_ADD, DX_BLEND_ONE, DX_BLEND_ZERO, DX_BLENDOP_ADD, GSYS.DrawSetting.NotWriteAlphaChannelFlag ) ;
		}
		break ;
	...
	case DX_BLENDMODE_ADD :
		if( Frame->Container->UseDrawMulAlphaColor )
		{
			Graphics_Linux_DeviceState_SetBlendMode( DX_BLENDMODE_PMA_ADD, FALSE, DX_BLEND_ONE, DX_BLEND_ZERO, DX_BLENDOP_ADD, DX_BLEND_ONE, DX_BLEND_ZERO, DX_BLENDOP_ADD, GSYS.DrawSetting.NotWriteAlphaChannelFlag ) ;
		}
		else
		{
			Graphics_Linux_DeviceState_SetBlendMode( DX_BLENDMODE_ADD, FALSE, DX_BLEND_ONE, DX_BLEND_ZERO, DX_BLENDOP_ADD, DX_BLEND_ONE, DX_BLEND_ZERO, DX_BLENDOP_ADD, GSYS.DrawSetting.NotWriteAlphaChannelFlag ) ;
		}
		break ;
	}

	// スペキュラが有効かどうかをセットする
	Graphics_Linux_DeviceState_SetSpecularEnable(
	...
	Graphics_Linux_DeviceState_SetUseVertexDiffuseColor( MBMesh->UseVertexDiffuseColor ) ;

	// スペキュラマテリアルとして頂点カラーを使用するかどうかをセット
	Graphics_Linux_DeviceState_SetUseVertexSpecularColor( MBMesh->UseVertexSpecularColor ) ;

	return true ;
}

// トゥーンの為のマテリアルをセットアップする
static void MV1_LINUX_SetupToonOutLineMeshDrawMaterialCommon( MV1_MESH *Mesh, float OutLineWidth )
{
//	MV1_MESH_BASE     *MBMesh ;
	MATERIALPARAM     Material ;
	MV1_MATERIAL      * RST MMaterial ;
...
//	MBMaterial = MMaterial->BaseData ;
	...
	// 輪郭線の太さをセットする
	Graphics_Linux_DeviceState_SetToonOutLineSize( OutLineWidth ) ;

	// カリング設定
	Graphics_Linux_DeviceState_SetCullMode( 2 ) ; 
	...
	// スペキュラを無効にする
	Graphics_Linux_DeviceState_SetSpecularEnable( FALSE ) ;
}

// グラフィックハンドルからテクスチャを取得する
static GRAPHICS_LINUX_TEXTURE	*MV1_LINUX_GetGraphHandleTexture( int GraphHandle )
{
	IMAGEDATA *Image ;
...
static GRAPHICS_LINUX_TEXTURE	*MV1_LINUX_GetShadowMapHandleTexture( int ShadowMapHandle )
{
...
static GRAPHICS_LINUX_TEXTURE	*MV1_LINUX_GetTextureTexture( MV1_TEXTURE *Tex )
{
	...
	return MV1_LINUX_GetGraphHandleTexture( GraphHandle ) ;
}

// テクスチャが無かった場合にセットするテクスチャを取得する
static GRAPHICS_LINUX_TEXTURE	*MV1_LINUX_GetNoneGraphHandleTexture( void )
{
	...
	return MV1_LINUX_GetGraphHandleTexture( MV1Man.TexNoneHandle ) ;
}

// 指定のメッシュのマテリアルをセットアップする
static void MV1_LINUX_SetupMeshDrawMaterial( MV1_MESH *Mesh )
{
	MV1_TEXTURE_BASE			*TexBase ;
	MV1_TEXTURE					*Tex ;
	MV1_FRAME					*Frame ;
...
//	int							BlendMode ;
	...
	float						TextureScaleU ;
	float						TextureScaleV ;
	GRAPHICS_LINUX_TEXTURE	*SetTexture[ 16 ] = { NULL } ;
	int							SetTextureNum = 0 ;
	int							IsToon ;
...
//	BlendMode = Mesh->DrawBlendMode != -1 ? Mesh->DrawBlendMode : MMaterial->DrawBlendMode ;
	...
	if( Frame->TextureAddressTransformUse == FALSE && TextureUVScaling == FALSE )
	{
		...
		Graphics_Linux_DeviceState_SetTextureAddressTransformMatrix( FALSE, NULL ) ;
	}
	else
	{
		GSYS.DrawSetting.MatchHardwareTextureAddressTransformMatrix = FALSE ;
		...
		if( TextureUVScaling )
		{
			...
			Graphics_Linux_DeviceState_SetTextureAddressTransformMatrix( TRUE, &TempMatrix ) ;
		}
		else
		{
			Graphics_Linux_DeviceState_SetTextureAddressTransformMatrix( TRUE, &Frame->TextureAddressTransformMatrix ) ;
		}
	}
	...
	// テクスチャのセットアップ
	{
		...
		{
			Tex = &Model->Texture[ Mesh->Material->DiffuseLayer[ 0 ].Texture ] ;
			SetTexture[ 0 ] = MV1_LINUX_GetTextureTexture( Tex ) ;
			Graphics_Linux_DeviceState_SetTextureAddressUVW( Tex->AddressModeU, Tex->AddressModeV, DX_TEXADDRESS_CLAMP, 0 ) ;
			Graphics_Linux_DeviceState_SetSampleFilterMode( DrawModeToFilterTable[ SetTexture[ 0 ]->MipMapCount <= 1 ? 0 : 1 ][ Tex->FilterMode ], 0 ) ;
		}
		else
		{
			SetTexture[ 0 ] = MV1_LINUX_GetNoneGraphHandleTexture() ;
			Graphics_Linux_DeviceState_SetSampleFilterMode( GL_NEAREST, 0 ) ;
		}
		SetTextureNum = 1 ;
		...
		if( Mesh->BaseData->Material->NormalLayerNum )
		{
			...
			SetTexture[ 1 ] = MV1_LINUX_GetTextureTexture( Tex ) ;
			Graphics_Linux_DeviceState_SetTextureAddressUVW( Tex->AddressModeU, Tex->AddressModeV, DX_TEXADDRESS_CLAMP, 1 ) ;
			Graphics_Linux_DeviceState_SetSampleFilterMode( DrawModeToFilterTable[ SetTexture[ 1 ]->MipMapCount <= 1 ? 0 : 1 ][ Tex->FilterMode ], 1 ) ;
		}
		...
		if( Mesh->Material->SpecularLayerNum )
		{
			...
			SetTexture[ 2 ] = MV1_LINUX_GetTextureTexture( Tex ) ;
			Graphics_Linux_DeviceState_SetTextureAddressUVW( Tex->AddressModeU, Tex->AddressModeV, DX_TEXADDRESS_CLAMP, 2 ) ;
			Graphics_Linux_DeviceState_SetSampleFilterMode( DrawModeToFilterTable[ SetTexture[ 2 ]->MipMapCount <= 1 ? 0 : 1 ][ Tex->FilterMode ], 2 ) ;
		}
		else
		{
			SetTextureNum = 3 ;

			SetTexture[ 2 ] = MV1_LINUX_GetNoneGraphHandleTexture() ;
			Graphics_Linux_DeviceState_SetSampleFilterMode( GL_NEAREST, 2 ) ;
		}
		...
		if( IsToon )
		{
			...
			if( Mesh->Material->DiffuseGradTexture >= 0 )
			{
				...
				SetTexture[ 3 ] = MV1_LINUX_GetTextureTexture( Tex ) ;
				Graphics_Linux_DeviceState_SetTextureAddressUVW( Tex->AddressModeU, Tex->AddressModeV, DX_TEXADDRESS_CLAMP, 3 ) ;
				Graphics_Linux_DeviceState_SetSampleFilterMode( DrawModeToFilterTable[ SetTexture[ 3 ]->MipMapCount <= 1 ? 0 : 1 ][ Tex->FilterMode ], 3 ) ;
			}
			else
			if( Mesh->Material->BaseData->Type == DX_MATERIAL_TYPE_TOON )
			{
				SetTexture[ 3 ] = MV1_LINUX_GetGraphHandleTexture( MV1GetDefaultToonTexture( Mesh->Material->DiffuseGradTexture ) ) ;
				Graphics_Linux_DeviceState_SetSampleFilterMode( GL_LINEAR, 3 ) ;
			}
			else
			{
				SetTexture[ 3 ] = MV1_LINUX_GetNoneGraphHandleTexture() ;
				Graphics_Linux_DeviceState_SetSampleFilterMode( GL_NEAREST, 3 ) ;
			}

			if( Mesh->Material->SpecularGradTexture >= 0 )
			{
				Tex = &Model->Texture[ Mesh->Material->SpecularGradTexture ] ;
				SetTexture[ 4 ] = MV1_LINUX_GetTextureTexture( Tex ) ;
				Graphics_Linux_DeviceState_SetTextureAddressUVW( Tex->AddressModeU, Tex->AddressModeV, DX_TEXADDRESS_CLAMP, 4 ) ;
				Graphics_Linux_DeviceState_SetSampleFilterMode( DrawModeToFilterTable[ SetTexture[ 4 ]->MipMapCount <= 1 ? 0 : 1 ][ Tex->FilterMode ], 4 ) ;
			}
			else
			{
				SetTexture[ 4 ] = MV1_LINUX_GetGraphHandleTexture( MV1GetDefaultToonTexture( Mesh->Material->SpecularGradTexture ) ) ;
				Graphics_Linux_DeviceState_SetSampleFilterMode( GL_LINEAR, 4 ) ;
			}
			...
			if( Mesh->Material->SphereMapTexture >= 0 )
			{
				...
				if( Tex->UseUserGraphHandle )
				{
					SetTexture[ 5 ] = MV1_LINUX_GetGraphHandleTexture( Tex->UserGraphHandle ) ;
				}
				else
				if( Tex->UseGraphHandle && Tex->IsDefaultTexture == FALSE )
				{
					SetTexture[ 5 ] = MV1_LINUX_GetGraphHandleTexture( Tex->GraphHandle ) ;
				}
				else
				if( TexBase->UseUserGraphHandle )
				{
					SetTexture[ 5 ] = MV1_LINUX_GetGraphHandleTexture( TexBase->UserGraphHandle ) ;
				}
				else
				if( TexBase->IsDefaultTexture == FALSE )
				{
					SetTexture[ 5 ] = MV1_LINUX_GetGraphHandleTexture( TexBase->GraphHandle ) ;
				}
				else
				{
					if( MV1Man.TexNoneHandle < 0 )
					...
					{
						SetTexture[ 5 ] = MV1_LINUX_GetGraphHandleTexture( MV1Man.TexNoneHandle ) ;
					}
					else
					{
						SetTexture[ 5 ] = MV1_LINUX_GetGraphHandleTexture( MV1Man.TexNoneBlackHandle ) ;
					}
				}
				Graphics_Linux_DeviceState_SetTextureAddressUVW( Tex->AddressModeU, Tex->AddressModeV, DX_TEXADDRESS_CLAMP, 5 ) ;
				Graphics_Linux_DeviceState_SetSampleFilterMode( DrawModeToFilterTable[ SetTexture[ 5 ]->MipMapCount <= 1 ? 0 : 1 ][ Tex->FilterMode ], 5 ) ;
			}


//			SetTextureNum = 7 ;
//			SetTexture[ 6 ] = &GLINUX.RGBtoVMaxRGBVolumeTexture ;
//			Graphics_Linux_DeviceState_SetTextureAddressUVW( DX_TEXADDRESS_CLAMP, DX_TEXADDRESS_CLAMP, DX_TEXADDRESS_CLAMP, 6 ) ;
//			Graphics_Linux_DeviceState_SetSampleFilterMode( Gnm::kFilterModeBilinear, 6 ) ;
		}
		...
		if( Mesh->Material->DiffuseLayerNum > 1 )
		{
			...
			SetTexture[ 7 ] = MV1_LINUX_GetTextureTexture( Tex ) ;
			Graphics_Linux_DeviceState_SetTextureAddressUVW( Tex->AddressModeU, Tex->AddressModeV, DX_TEXADDRESS_CLAMP, 7 ) ;
			Graphics_Linux_DeviceState_SetSampleFilterMode( DrawModeToFilterTable[ SetTexture[ 7 ]->MipMapCount <= 1 ? 0 : 1 ][ Tex->FilterMode ], 7 ) ;
		}
		...
		// シャドウマップの設定
		{
			...
			for( i = 0 ; i < MAX_USE_SHADOWMAP_NUM ; i ++ )
			{
				...
				{
					SetTexture[ 8 + i ] = MV1_LINUX_GetShadowMapHandleTexture( GSYS.DrawSetting.ShadowMap[ i ])  ;
					Graphics_Linux_DeviceState_SetTextureAddressUVW( DX_TEXADDRESS_CLAMP, DX_TEXADDRESS_CLAMP, DX_TEXADDRESS_CLAMP, 8 + i ) ;
					Graphics_Linux_DeviceState_SetSampleFilterMode( GL_LINEAR, 8 + i ) ;
				}
				else
				{
					SetTexture[ 8 + i ] = NULL ;
		...
		if( MV1Man.UseOrigShaderFlag )
		{
			...
			for( i = 0 ; i < USE_TEXTURESTAGE_NUM ; i ++ )
			{
				...
				LastNo = i ;
				Graphics_Linux_DeviceState_SetTextureAddressUVW( GSYS.DrawSetting.TexAddressModeU[ i ], GSYS.DrawSetting.TexAddressModeV[ i ], GSYS.DrawSetting.TexAddressModeW[ i ], i ) ;
				Graphics_Linux_DeviceState_SetSampleFilterMode( DrawModeToFilterTable[ SetTexture[ i ]->MipMapCount <= 1 ? 0 : 1 ][ GSYS.DrawSetting.DrawMode ], i ) ;
			}
		...
		// テクスチャをセット
		{
			...
			{
				Graphics_Linux_DeviceState_SetTexture( i, SetTexture[ i ] ) ;
			}
		}
	}
}
...
static void MV1_LINUX_SetupToonOutLineMeshDrawMaterial(	MV1_MESH *Mesh )
{
//	MV1_FRAME					*Frame ;
//	MV1_MODEL					*Model ;
//	MV1_MATERIAL				* RST MMaterial ;
//	int							BlendMode ;
	GRAPHICS_LINUX_TEXTURE	*SetTexture = NULL ;
...
//	BlendMode = Mesh->DrawBlendMode != -1 ? Mesh->DrawBlendMode : MMaterial->DrawBlendMode ;
	...
	// テクスチャのセットアップ
	SetTexture = MV1_LINUX_GetNoneGraphHandleTexture() ;

	// テクスチャをセット
	Graphics_Linux_DeviceState_SetTexture( 0, SetTexture ) ;
}

// 指定のモデル描画用シェーダーをセットアップする( TRUE:成功  FALSE:失敗 )
static int  MV1_LINUX_ShaderSetup( MV1_MESH *Mesh, int VertexType, int IsToonOutLine )
{
	GRAPHICS_HARDWARE_LINUX_SHADER_MODEL		*SM           = &GLINUX.Device.Shader.Model ;
	GRAPHICS_HARDWARE_LINUX_SHADERCODE_MODEL	*SCM          = &GLINUX.ShaderCode.Model ;
	int											ValidPL ;
	GRAPHICS_LINUX_SHADER						**Shader_PL   = NULL ;
	GLuint										*VS_PL        = NULL ;
	GRAPHICS_HARDWARE_LINUX_SHADERCODE_INFO	*VSAddress_PL = NULL ;
	GLuint										*FS_PL        = NULL ;
	GRAPHICS_HARDWARE_LINUX_SHADERCODE_INFO	*FSAddress_PL = NULL ;
	GRAPHICS_LINUX_SHADER						**Shader      = NULL ;
	GLuint										*VS           = NULL ;
	GRAPHICS_HARDWARE_LINUX_SHADERCODE_INFO	*VSAddress    = NULL ;
	GLuint										*FS           = NULL ;
	GRAPHICS_HARDWARE_LINUX_SHADERCODE_INFO	*FSAddress    = NULL ;
	MV1_MATERIAL_BASE                       * RST MBMaterial ;
	MV1_MATERIAL                            * RST MMaterial ;
	int										ShadowMap ;
	int										FogType ;
...
//	int                                     SpecularMap ;
	...
	if( GLINUX.ShaderCode.Model.ModelShaderInitializeFlag == FALSE &&
		Graphics_Linux_ShaderCode_Model_Initialize()   == FALSE )
	{
		return FALSE ;
	}

	MV1Man_Linux.EnableLightNum = 0 ;
	...
	if( MMaterial->UseAlphaTest )
	{
		AlphaTestMode = Graphics_Linux_Shader_GetAlphaTestModeIndex( MMaterial->UseAlphaTest, MMaterial->AlphaFunc ) ;
		Graphics_Linux_DeviceState_SetAlphaTestRef( MMaterial->AlphaRef ) ;
	}
	else
	{
		AlphaTestMode = LINUX_FS_ALPHATEST_CMP_GREATER ;
		Graphics_Linux_DeviceState_SetAlphaTestRef( -1 ) ;
	}
	ShadowMap     = GSYS.DrawSetting.UseShadowMapNum != 0 ? 1 : 0 ;
	FogType       = GLINUX.Device.State.FogEnable ? GLINUX.Device.State.FogMode : DX_FOGMODE_NONE ;
...
//	SpecularMap   = MMaterial->SpecularLayerNum > 0 ;
	...
	if( GLINUX.Device.State.Lighting )
	{
		// ライティングあり描画
		...
		{
			if( GLINUX.Device.State.LightEnableFlag[ i ] )
			{
				LightMode[ i ] = GLINUX.Device.State.LightParam[ i ].LightType ;
				EnableLightNum ++ ;
			}
			else
			{
...
//		LightIndex84 = GLINUX.ShaderCode.Base.LightIndexList84[ LightMode[ 0 ] ][ LightMode[ 1 ] ][ LightMode[ 2 ] ][ LightMode[ 3 ] ][ LightMode[ 4 ] ][ LightMode[ 5 ] ] ;
		LightIndex20 = GLINUX.ShaderCode.Base.LightIndexList20[ LightMode[ 0 ] ][ LightMode[ 1 ] ][ LightMode[ 2 ] ] ;
		LightIndex10 = GLINUX.ShaderCode.Base.LightIndexList10
...
//		{
//			if( Graphics_Linux_DeviceState_SetVertexShaderToHandle( GSYS.DrawSetting.UserShaderRenderInfo.SetVertexShaderHandle ) < 0 )
//			{
//				UseOrigShader = FALSE ;
//			}
//		}
...
//		{
//			if( Graphics_Linux_DeviceState_SetPixelShaderToHandle( GSYS.DrawSetting.UserShaderRenderInfo.SetPixelShaderHandle ) < 0 )
//			{
//				UseOrigShader = FALSE ;
//			}
//		}
//	}
	...
	if( UseOrigShader == FALSE )
	{
		if( ( GSYS.Light.EnableNum > DX_VERTEXLIGHTING_LIGHT_NUM || GLINUX.UsePixelLightingShader ) && ValidPL )
		{
			...
			if( *Shader_PL == NULL )
			{
				*Shader_PL = Graphics_Linux_GetShaderStruct() ;
				if( *Shader_PL == NULL )
				{
					return FALSE ;
				}
			...
			{
				// シェーダーの作成を試みる
				if( Graphics_Linux_Shader_Create( *Shader_PL, *VS_PL, *FS_PL ) != 0 )
				{
					DXST_LOGFILEFMT_ADDUTF16LE(( ""/*@ L"MV1_LINUX_ShaderSetup でピクセルライティング用シェーダーの作成に失敗しました\n" @*/ )) ;
					return FALSE ;
				}
			}

			MV1Man_Linux.EnableLightNum = EnableLightNum ;

			// シェーダーのセットアップ
			Graphics_Linux_DeviceState_SetShader( *Shader_PL, FALSE ) ;
		}
		else
		{
			// シェーダーがあるかどうかを調べる
			...
			{
				// シェーダーの作成を試みる
				if( Graphics_Linux_VertexShader_Create( VSAddress, VS, 1 ) != 0 )
				{
					DXST_LOGFILEFMT_ADDUTF16LE(( ""/*@ L"MV1_LINUX_ShaderSetup で頂点ライティング用頂点シェーダーの作成に失敗しました\n" @*/ )) ;
					return FALSE ;
				}
			}
			...
			if( *Shader == NULL )
			{
				*Shader = Graphics_Linux_GetShaderStruct() ;
				if( *Shader == NULL )
				{
					return FALSE ;
				}
			...
			{
				// シェーダーの作成を試みる
				if( Graphics_Linux_Shader_Create( *Shader, *VS, *FS ) != 0 )
				{
					DXST_LOGFILEFMT_ADDUTF16LE(( ""/*@ L"MV1_LINUX_ShaderSetup で頂点ライティング用シェーダーの作成に失敗しました\n" @*/ )) ;
					return FALSE ;
				}
			}

			MV1Man_Linux.EnableLightNum = EnableLightNum > 3 ? 3 : EnableLightNum ;

			// シェーダーのセットアップ
			Graphics_Linux_DeviceState_SetShader( *Shader, FALSE ) ;
		}
	}
...
static void MV1_LINUX_SetWorldMatrix_MATRIX_4X4CT( const MATRIX_4X4CT *Matrix, int UseWorldViewMatrix )
{
	// 定数データに反映
	if( GSYS.DrawSetting.Large3DPositionSupport )
	{
		...
		{
			GLINUX.Device.Shader.Constant.uLWMat[ 0 ][ 0 ] = ( float )Matrix->md.m[ 0 ][ 0 ] ;
			GLINUX.Device.Shader.Constant.uLWMat[ 0 ][ 1 ] = ( float )Matrix->md.m[ 0 ][ 1 ] ;
			GLINUX.Device.Shader.Constant.uLWMat[ 0 ][ 2 ] = ( float )Matrix->md.m[ 0 ][ 2 ] ;
			GLINUX.Device.Shader.Constant.uLWMat[ 0 ][ 3 ] = ( float )Matrix->md.m[ 0 ][ 3 ] ;
			GLINUX.Device.Shader.Constant.uLWMat[ 1 ][ 0 ] = ( float )Matrix->md.m[ 1 ][ 0 ] ;
			GLINUX.Device.Shader.Constant.uLWMat[ 1 ][ 1 ] = ( float )Matrix->md.m[ 1 ][ 1 ] ;
			GLINUX.Device.Shader.Constant.uLWMat[ 1 ][ 2 ] = ( float )Matrix->md.m[ 1 ][ 2 ] ;
			GLINUX.Device.Shader.Constant.uLWMat[ 1 ][ 3 ] = ( float )Matrix->md.m[ 1 ][ 3 ] ;
			GLINUX.Device.Shader.Constant.uLWMat[ 2 ][ 0 ] = ( float )Matrix->md.m[ 2 ][ 0 ] ;
			GLINUX.Device.Shader.Constant.uLWMat[ 2 ][ 1 ] = ( float )Matrix->md.m[ 2 ][ 1 ] ;
			GLINUX.Device.Shader.Constant.uLWMat[ 2 ][ 2 ] = ( float )Matrix->md.m[ 2 ][ 2 ] ;
			GLINUX.Device.Shader.Constant.uLWMat[ 2 ][ 3 ] = ( float )Matrix->md.m[ 2 ][ 3 ] ;
		}
		else
		{
			...
			GLINUX.Device.Shader.Constant.uLWMat[ 0 ][ 0 ] = ( float )TempMat.md.m[ 0 ][ 0 ] ;
			GLINUX.Device.Shader.Constant.uLWMat[ 0 ][ 1 ] = ( float )TempMat.md.m[ 0 ][ 1 ] ;
			GLINUX.Device.Shader.Constant.uLWMat[ 0 ][ 2 ] = ( float )TempMat.md.m[ 0 ][ 2 ] ;
			GLINUX.Device.Shader.Constant.uLWMat[ 0 ][ 3 ] = ( float )TempMat.md.m[ 0 ][ 3 ] ;
			GLINUX.Device.Shader.Constant.uLWMat[ 1 ][ 0 ] = ( float )TempMat.md.m[ 1 ][ 0 ] ;
			GLINUX.Device.Shader.Constant.uLWMat[ 1 ][ 1 ] = ( float )TempMat.md.m[ 1 ][ 1 ] ;
			GLINUX.Device.Shader.Constant.uLWMat[ 1 ][ 2 ] = ( float )TempMat.md.m[ 1 ][ 2 ] ;
			GLINUX.Device.Shader.Constant.uLWMat[ 1 ][ 3 ] = ( float )TempMat.md.m[ 1 ][ 3 ] ;
			GLINUX.Device.Shader.Constant.uLWMat[ 2 ][ 0 ] = ( float )TempMat.md.m[ 2 ][ 0 ] ;
			GLINUX.Device.Shader.Constant.uLWMat[ 2 ][ 1 ] = ( float )TempMat.md.m[ 2 ][ 1 ] ;
			GLINUX.Device.Shader.Constant.uLWMat[ 2 ][ 2 ] = ( float )TempMat.md.m[ 2 ][ 2 ] ;
			GLINUX.Device.Shader.Constant.uLWMat[ 2 ][ 3 ] = ( float )TempMat.md.m[ 2 ][ 3 ] ;

			Graphics_Linux_DeviceState_SetViewMatrix( &IdentityMat ) ;
		}
	}
	else
	{
		GLINUX.Device.Shader.Constant.uLWMat[ 0 ][ 0 ] = Matrix->mf.m[ 0 ][ 0 ] ;
		GLINUX.Device.Shader.Constant.uLWMat[ 0 ][ 1 ] = Matrix->mf.m[ 0 ][ 1 ] ;
		GLINUX.Device.Shader.Constant.uLWMat[ 0 ][ 2 ] = Matrix->mf.m[ 0 ][ 2 ] ;
		GLINUX.Device.Shader.Constant.uLWMat[ 0 ][ 3 ] = Matrix->mf.m[ 0 ][ 3 ] ;
		GLINUX.Device.Shader.Constant.uLWMat[ 1 ][ 0 ] = Matrix->mf.m[ 1 ][ 0 ] ;
		GLINUX.Device.Shader.Constant.uLWMat[ 1 ][ 1 ] = Matrix->mf.m[ 1 ][ 1 ] ;
		GLINUX.Device.Shader.Constant.uLWMat[ 1 ][ 2 ] = Matrix->mf.m[ 1 ][ 2 ] ;
		GLINUX.Device.Shader.Constant.uLWMat[ 1 ][ 3 ] = Matrix->mf.m[ 1 ][ 3 ] ;
		GLINUX.Device.Shader.Constant.uLWMat[ 2 ][ 0 ] = Matrix->mf.m[ 2 ][ 0 ] ;
		GLINUX.Device.Shader.Constant.uLWMat[ 2 ][ 1 ] = Matrix->mf.m[ 2 ][ 1 ] ;
		GLINUX.Device.Shader.Constant.uLWMat[ 2 ][ 2 ] = Matrix->mf.m[ 2 ][ 2 ] ;
		GLINUX.Device.Shader.Constant.uLWMat[ 2 ][ 3 ] = Matrix->mf.m[ 2 ][ 3 ] ;
	}

	GLINUX.Device.Shader.Constant.UpdateCount ++ ;
}

// モデル描画専用の ローカル　→　ワールド行列をセットアップする
static void MV1_LINUX_SetupModelDrawWorldMatrix( MV1_FRAME *Frame, MV1_TRIANGLE_LIST_BASE *MBTList, int UseWorldViewMatrix )
{
	MATRIX_4X4CT * RST SrcMatrix ;
	DX_LINUX_SHADER_FLOAT4 *Dest ;
	int i ;

	// 定数データに反映
	Dest = GLINUX.Device.Shader.Constant.uLWMat ;
	if( GSYS.DrawSetting.Large3DPositionSupport )
	{
		if( UseWorldViewMatrix )
		{
...
static int MV1_LINUX_UpdateSkinFreeBoneMeshVertexData(
	MV1_FRAME_BASE         *MBFrame,
	MV1_TRIANGLE_LIST_BASE *MBTList,
	MV1_FRAME              *Frame,
	MV1_TRIANGLE_LIST      *TList
...
//	int                              SetupShaderMaterial ;
	...
	// マテリアルの共通部分のセットアップ
	if( MV1_LINUX_SetupMeshDrawMaterialCommon( Mesh ) == false )
	{
		return ;
	}

	// マテリアルのセットアップ
	MV1_LINUX_SetupMeshDrawMaterial( Mesh ) ;
...
//			MBTListPF = ( MV1_TRIANGLE_LIST_BASE_PF * )MBTList->PFBuffer ;
			...
			{
				if( MV1_LINUX_ShaderSetup( Mesh, MV1_VERTEX_TYPE_SKIN_4BONE, FALSE ) == FALSE )
				{
					continue ;
				}
			}
			...
			{
				if( MV1_LINUX_ShaderSetup( Mesh, MBTList->VertexType, FALSE ) == FALSE )
				{
					continue ;
				}
			}
			...
			case MV1_VERTEX_TYPE_NORMAL :		// 剛体トライアングルリスト
				...
				// 使用するローカル→ワールド行列の用意
				MV1_LINUX_SetWorldMatrix_MATRIX_4X4CT( &Frame->LocalWorldMatrix, UseWorldViewMatrix ) ;

#ifndef NOT_RENDER
				// ステートと定数バッファの更新
				Graphics_Linux_DeviceState_UpdateShaderUniform( GLINUX.Device.State.SetShader, MV1Man_Linux.EnableLightNum ) ;
...
//				Graphics_LINUX_RenderEnd() ;
				...
				{
					Graphics_Linux_DeviceState_SetViewMatrix( &GSYS.DrawSetting.ViewMatrixF ) ;
				}
				break ;
							...
							else
							{
								...
								for( l = 0 ; l < DrawStockNum ; l ++, PackDrawMatrix += MBase->PackDrawMatrixUnitNum )
								{
									for( n = 0 ; n < MBTList->UseBoneNum ; n ++, MatrixIndex ++ )
									{
										...
										Dest = &GLINUX.Device.Shader.Constant.uLWMat[ MatrixIndex * 3 ] ;
...
#ifndef NOT_RENDER
							// ステートと定数バッファの更新
							Graphics_Linux_DeviceState_UpdateShaderUniform( GLINUX.Device.State.SetShader, MV1Man_Linux.EnableLightNum, 162 ) ;
...
//							Graphics_LINUX_RenderEnd() ;
					...
					{
						MV1_LINUX_SetupModelDrawWorldMatrix( Frame, MBTList, UseWorldViewMatrix ) ;
#ifndef NOT_RENDER
						// ステートと定数バッファの更新
						Graphics_Linux_DeviceState_UpdateShaderUniform( GLINUX.Device.State.SetShader, MV1Man_Linux.EnableLightNum, MBTList->UseBoneNum * 3 ) ;
...
//						Graphics_LINUX_RenderEnd() ;
			...
			case MV1_VERTEX_TYPE_SKIN_FREEBONE :
				// ローカル→ワールド行列は単位行列にする
				Graphics_Linux_DeviceState_SetWorldMatrix( &IdentityMat ) ;

#ifndef NOT_RENDER
				// 頂点データを更新する
				MV1_LINUX_UpdateSkinFreeBoneMeshVertexData( MBFrame, MBTList, Frame, TList ) ;
...
//				Graphics_LINUX_RenderEnd() ;
		...
		GSYS.DrawSetting.ShadowMapDraw == FALSE )
	{
			...
			Mesh->Material->OutLineDotWidth > 0.000001f )
		{
			...
			// マテリアルの共通部分のセットアップ
			MV1_LINUX_SetupToonOutLineMeshDrawMaterialCommon( Mesh, OutLineWidth ) ;

			// 描画マテリアルのセットアップ
			MV1_LINUX_SetupToonOutLineMeshDrawMaterial( Mesh ) ;
...
//				MBTListPF = ( MV1_TRIANGLE_LIST_BASE_PF * )MBTList->PFBuffer ;
				...
				if( MBTList->VertexType == MV1_VERTEX_TYPE_NORMAL && MBase->UsePackDraw )
				{
					if( MV1_LINUX_ShaderSetup( Mesh, MV1_VERTEX_TYPE_SKIN_4BONE, TRUE ) == FALSE )
					{
						continue ;
					}
				}
				...
				{
					if( MV1_LINUX_ShaderSetup( Mesh, MBTList->VertexType, TRUE ) == FALSE )
					{
						continue ;
					}
				}
